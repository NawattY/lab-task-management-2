...
| Rule | Description |
|------|-------------|
| **Single `.env` at root** | No `.env` in sub-packages |
| **No secrets in code** | Use `process.env` always |
| **Type-safe env** | Use Zod validation in config/env.ts |
| **NEXT_PUBLIC_ prefix** | Client-side env in Next.js must have prefix |
